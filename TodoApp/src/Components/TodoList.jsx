...
const TodoList = ({ todo }) => {
  ...
  const handleCheckboxChange = (todo) => {
    setCheckedItems(prevState => ({ ...prevState, [todo]: !prevState[todo] }));
  };
  ...
  const displayFilter = (filter) => {
    if (filter === "Active") {
      const filteredList = Object.keys(checkedItems).filter(todo => !checkedItems[todo]);
        ...
        <div key={todoIndex} className={`p-4 mb-4 ${bgColor[todoIndex % bgColor.length]}`}>
          <div className="flex items-center justify-between rounded text-primary-content">
            <div className="flex items-center">
              ...
              />
              <span className={checkedItems[todo] ? 'line-through' : ''}>{todo}</span>
            </div>
            <button className="btn btn-circle p-3" onClick={() => deleteTask(todo)}>
